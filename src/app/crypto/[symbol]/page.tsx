...
export async function generateMetadata({
  ...
  return {
    title: `${upperSymbol} Price in AUD Today`,
    description: `Live-tracked ${upperSymbol} price in AUD and USD, chart, and market cap.`,
  };
}
...
export default async function CryptoAssetPage({ params }: { params: { symbol: string } }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      <h1 className="text-2xl font-semibold text-navy-900">{upperSymbol}</h1>
      ...
      <section className="mt-6 rounded-lg border border-slate-200 p-5">
        <p className="text-sm text-slate-700">Want to buy or trade {upperSymbol}?</p>
        <a
